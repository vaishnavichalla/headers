...
var h = x.httpRequest.headers.email;
console.log(h)
...
const connection = request.accept(null, request.origin);
    ...
    connection.on("message", message => {
        ...
        var return_message = "Number of consonants in "+ms+" are "+(len-count);
...
connection.send("Now, send a word to the server to know the number of consonants in it")
} 
...
{const errorMessage = {
    ...
    "Description": "A header with 'email' as key and your email ID as value is a required for connecting to this WebSocket Server"
};
    connection.send(JSON.stringify(errorMessage))
    connection.send("Configure the request correctly and try to connect again")
    connection.close()
...
"Description": "Value of the header passed ('email') is empty"
};
    connection.send(JSON.stringify(errorMessage))
    connection.send("Configure the request correctly and try to connect again")
    connection.close()
